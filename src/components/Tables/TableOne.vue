...
<template>
  <div class="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default">
    <h4 class="mb-6 text-xl font-semibold text-black">{{ title }}</h4>
    ...
    <div class="flex flex-col">
      ...
      >
        ...
        <div class="flex items-center justify-center p-2.5 xl:p-5 space-x-3">
          <button @click="emit('view', item)" class="text-blue-500 hover:underline text-xl">👁</button>
          <button @click="emit('edit', item)" class="text-yellow-500 hover:underline text-xl">✏</button>
          <button @click="emit('delete', item)" class="text-red-500 hover:underline text-xl">🗑</button>
        </div>
      </div>
    </div>
  </div>
